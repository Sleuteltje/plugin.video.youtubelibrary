...
def get_playlist(playlist_url, basic=False, gdata=False, signature=True,
    ...
    ########### Adaption by Sleuteltje ####################
    # The regex of pafy won't grab playlist ids of uploaded videos playlists. Therefor i supply my own regex
    #################################################
    regx = re.compile(r'list=(([-_0-9a-zA-Z]+))')
    m = regx.search(playlist_url)

    if not m:
        err = "Unrecognized playlist url: %s"
        #Fix to Recognize the playlist ID with a uploaded videos from a user to
